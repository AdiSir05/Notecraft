...
export const LyricsLine = ({ songId, sectionId, line }: LyricsLineProps) => {
  ...
  return (
    <div className="space-y-1 lyrics-line">
      {/* Display words with chords - this replaces the double display */}
      <div className="flex flex-wrap gap-x-1 min-h-[48px] items-end pb-2">
        {words.map((word) => (
          <div key={word.id} className="relative group">
            ...
            <span className="text-notecraft-brown">{word.text}</span>
          </div>
        ))}
      </div>
